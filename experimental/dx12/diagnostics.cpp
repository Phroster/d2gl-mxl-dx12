    ...
    for(;;) {
        ...
        if(milliseconds(now-last_summary)>=1000 || s.quitting.load()) {
            ...
            if(!_wfopen_s(&out,path.c_str(),L"wb") && out) {
                fprintf(out,"MXL private diagnostic beta 5\nstate=%s\nrecords=%llu\nslow_frames=%llu\ndropped_records=%llu\n",
...
bool start(HWND window,const std::wstring& test_directory) {
    ...
    if(test_directory.empty() && !GetPrivateProfileIntW(L"Diagnostics",L"enabled",1,ini.c_str()))return false;
    ...
    if(!_wfopen_s(&file,(s.directory+L"\\session.txt").c_str(),L"wb") && file) {
        fprintf(file,"MXL private diagnostics beta 5\npid=%lu\nqpc_frequency=%lld\nqpc_start=%llu\n",
