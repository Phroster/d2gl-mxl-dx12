...
Performance recording is off unless `[Diagnostics] enabled=1` is explicitly set in `mxl-diagnostics.ini`. With recording off, no diagnostic writer, audio timing hooks, reveal timing detours or GPU timestamp queries are started. The upload fix, guarded automatic reveal and tile cache remain active. A process-wide owner prevents duplicate reveal initialization across the two renderer DLLs.
