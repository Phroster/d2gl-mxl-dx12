...
Performance recording is off unless `[Diagnostics] enabled=1` is explicitly set in `mxl-diagnostics.ini`. With recording off, no diagnostic writer, audio timing hooks, reveal timing detours or GPU timestamp queries are started. The upload fix, guarded automatic reveal, asset caches and sound-cancellation wakeup remain active. A process-wide owner prevents duplicate reveal initialization across the two renderer DLLs.
...
This fix is enabled for `Game.exe` independently of recording. Set `[Other] sound_cancel_wakeup=0` in `d2gl.ini` to disable it on the next launch. When recording is on, `sound_cancel_fix_ready=1` confirms activation and `sound_cancel_wakeup` records an eligible cancellation wake. The release wrapper also records `async_free` duration in `native-sound.csv`, including the native completion wait. Native sound-probe and cancellation imports install separately; a probe installation message alone does not confirm that the fix is active.
