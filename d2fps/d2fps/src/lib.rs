...
struct InstanceSync {
  ...
  /// Paired client/source clock correction, active only after guarded installation.
  mxl_timing: mxl_timing::MxlTiming,
...
static INSTANCE: Instance = Instance {
  sync: Mutex::new(InstanceSync {
    ...
    mxl_timing: mxl_timing::MxlTiming::new(),
    delayed: None,
  }),
  config: Config::new(),
...
unsafe extern "stdcall" fn Init() {
  ...
  loop {
    ...
    if !is_attached {
      ...
      log!("Attaching D2fps...");
      log!("D2FPS MXL 1.0: precise clocks and bounded realm prediction");
