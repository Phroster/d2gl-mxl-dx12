...
impl InstanceSync {
  pub fn attach(&mut self) {
    ...
    if version == "v1.13c" && INSTANCE.config.features.motion_smoothing() {
      self.mxl_timing =
        unsafe { crate::mxl_timing::MxlTiming::attach(modules[d2::Module::Client]) };
    }
  ...
  fn reapply_patches(&mut self) {
    if let Some((patches, modules)) = self.reapply_patches.take() {
      ...
      if !unsafe { self.mxl_timing.reapply() } {
        INSTANCE.config.features.remove_relaxed(Features::MotionSmoothing);
        log!("Disabling motion smoothing: MXL client clocks no longer verify");
      }
    }
  }
}
...
impl InstanceSync {
  ...
  unsafe fn update_game_time(&mut self, time: i64) -> bool {
    ...
    if self.game_update_time_ms != prev_update_time {
      let cur_time_ms = self.mxl_timing.current_time_ms(is_sp);
  ...
  unsafe fn update_entity_positions<T: Entity>(&mut self) {
    ...
    let offset =
      self
        .mxl_timing
        .interpolation_offset(since_update, frame_len, self.accessor.game_type());
