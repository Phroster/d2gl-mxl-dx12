...
void Context::beginFrame()
{
    ...
    if(mxl::diag::enabled() && App.game.screen==GameScreen::InGame) {
        ...
        static_assert(sizeof(MxlMotionSnapshot)==120);
        ...
        if(MxlSmoothing_ReadMotion(&motion)) {
            ...
            mxl::diag::producer_set(Count::MotionRenderRawTicks,motion.render_raw_ticks);
            mxl::diag::producer_set(Count::MotionRenderNowTicks,motion.render_now_ticks);
            mxl::diag::producer_set(Count::MotionRenderResets,motion.render_resets);
            mxl::diag::producer_set(Count::MotionRenderActive,motion.render_active);
        }
