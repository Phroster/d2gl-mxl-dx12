...
constexpr unsigned object_indicator_limit=12;
enum class ObjectKind : uint8_t { None,Container,Shrine,Special,Well,Waypoint,Stash };
struct ObjectFacts {
...
inline ObjectLook object_look(const ObjectFacts& f) {
    if(!f.drawn || !f.selectable || f.door || f.mode>=8) return {};
    // Waypoints and personal stashes remain useful after activation. Other
    // markers disappear immediately when the native object leaves neutral.
    if((f.subclass&0x40) || f.operate==23) return {ObjectKind::Waypoint,1,3,0,8};
    if(f.operate==32) return {ObjectKind::Stash,1,0,0,7};
    if(f.mode!=0) return {};
    if((f.subclass&1) || f.operate==2) return {ObjectKind::Shrine,2,3,1,24};
    ...
    if(f.operate==3 || f.operate==5) return {ObjectKind::Container,1,5,1,4};
    if(f.operate==7 || f.operate==30 || f.operate==68) return {ObjectKind::Container,1,4,1,3};
    switch(f.operate) {
        case 21:case 28:case 33:case 39:case 40:case 41:
        case 49:case 57:case 58:case 59:
            return {ObjectKind::Special,2,1,2,28};
    }
    ...
    if(!container) return {}; // No doors, torches, portals or scenery.
